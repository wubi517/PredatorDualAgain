...
public abstract class MovieCategoryAdapter extends RecyclerView.Adapter<MovieCategoryAdapter.HomeListViewHolder> {
    ...
    private Function4<Integer, CategoryModel, List<MovieModel>, Boolean, Unit> clickListenerFunction;

    MovieCategoryAdapter(Context context, Function4<Integer, CategoryModel, List<MovieModel>, Boolean, Unit> clickListenerFunction) {
...
//        if (position == 0) holder.itemView.requestFocus();
        ...
        holder.items_recyclerview.setAdapter(new MovieAdapter(movieModels, (pos, items, b) -> {
            clickListenerFunction.invoke(pos,list.get(position), items, b);
            return null;
        }));
    }
