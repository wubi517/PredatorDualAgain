...
public abstract class SeriesCategoryAdapter extends RecyclerView.Adapter<SeriesCategoryAdapter.HomeListViewHolder> {
    ...
    private Function4<Integer, CategoryModel, SeriesModel,Boolean, Unit> clickListenerFunction;
    SeriesCategoryAdapter( Context context, Function4<Integer, CategoryModel, SeriesModel,Boolean, Unit> clickListenerFunction) {
    ...
    public void onBindViewHolder(@NonNull HomeListViewHolder holder, final int position) {
        ...
        holder.itemsRecyclerView.setAdapter(new SeriesAdapter(seriesModels, (pos, item, isClicked) -> {
            clickListenerFunction.invoke(position,list.get(position), item, isClicked);
            return null;
        }));
    }
