...
import com.it_tech613.predator.ijklib.widget.media.AndroidMediaController;
import com.it_tech613.predator.ijklib.widget.media.IjkVideoView;
...
public class LiveIjkPlayActivity extends AppCompatActivity implements  SeekBar.OnSeekBarChangeListener,View.OnClickListener, IMediaPlayer.OnErrorListener,IMediaPlayer.OnCompletionListener,
    ...
    private void getEpg(){
        try {
            String epg = MyApp.instance.getIptvclient().getShortEPG(MyApp.user, MyApp.pass,
                    mStream_id+"",4);
            Log.e(getClass().getSimpleName(),epg);
            Gson gson=new Gson();
            epg=epg.replaceAll("[^\\x00-\\x7F]", "");
            if (!epg.contains("null_error_response")){
                Log.e("response",epg);
                try {
