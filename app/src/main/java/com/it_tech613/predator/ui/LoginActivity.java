...
import com.it_tech613.predator.models.MacCategoryModel;
...
//                id = "-1";
    ...
    private void callLogin() {
        try {
            ...
            JSONObject u_m;
            try {
                u_m = jsonObject.getJSONObject("user_info");
                if (!u_m.has("username")) {
                    runOnUiThread(()->{
                    ...
                    JSONObject serverInfo= jsonObject.getJSONObject("server_info");
    ...
    private void callVodCategory(){
        try {
            long startTime = System.nanoTime();
            //api call here
            String response = MyApp.instance.getIptvclient().getMovieCategories(user,password);
            long endTime = System.nanoTime();
...
//            categories.add(new CategoryModel(Constants.no_name_id, Constants.No_Name_Category,""));
            categories.addAll(gson.fromJson(response, new TypeToken<List<CategoryModel>>(){}.getType()));
    ...
    private void callLiveCategory(){
        try {
            long startTime = System.nanoTime();
//api call here
            String response = MyApp.instance.getIptvclient().getLiveCategories(user,password);
            long endTime = System.nanoTime();
    ...
    private void callSeriesCategory(){
        try {
            long startTime = System.nanoTime();
//api call here
            String response = MyApp.instance.getIptvclient().getSeriesCategories(user,password);
            long endTime = System.nanoTime();
...
//            categories.add(new CategoryModel(Constants.no_name_id, Constants.No_Name_Category,""));
    ...
    private void callLiveStreams(){
        try{
            long startTime = System.nanoTime();
//api call here
            String liveStreams = MyApp.instance.getIptvclient().getLiveStreams(user,password);
            long endTime = System.nanoTime();
           ...
           // Log.e(getClass().getSimpleName(),map);
            ...
            Gson gson=new Gson();
            liveStreams = liveStreams.replaceAll("[^\\x00-\\x7F]", "");
            ...
            try{
                ...
                JSONArray response;
                try {
                    response=new JSONArray(liveStreams);
...
//                }
    ...
    private void callSeries() {
        try {
            long startTime = System.nanoTime();
            //api call here
            String response = MyApp.instance.getIptvclient().getSeries(user,password);
            long endTime = System.nanoTime();
...
//            allSeriesModels.addAll((Collection<? extends SeriesModel>) gson.fromJson(map, new TypeToken<List<SeriesModel>>(){}.getType()));
    ...
    private void callMovieStreams() {
        try {
            long startTime = System.nanoTime();
//api call here
            String response = MyApp.instance.getIptvclient().getMovies(user,password);
            long endTime = System.nanoTime();
            ...
            Gson gson=new Gson();
            response = response.replaceAll("[^\\x00-\\x7F]", "");
            List<MovieModel> movieModels = new ArrayList<>(gson.fromJson(response, new TypeToken<List<MovieModel>>() {}.getType()));
